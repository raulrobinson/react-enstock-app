...
export const LoginUser = (email: string, password: string, router: AppRouterInstance) => {
    signInWithEmailAndPassword(auth, email, password)
        .then((userCredential) => {
            ...
            successMessage("Ingreso satisfactorio! 🎉");
...
export const LogOut = (router: AppRouterInstance) => {
	signOut(auth)
		...
		.catch((error) => {
			errorMessage("No ha logrado iniciar sesion! ❌");
		});
};
...
export const deleteCategory =  async (id: string, name:string) => {
	try {
		...
		successMessage(`${name} categoria eliminada! 🎉`)
	} catch (err) {
		errorMessage("Hemos encontrado un error! ❌")
		console.log(err)
	}
...
export const addCategory = async (name: string) => {
	try {
		...
		successMessage(`${name} categoria agregada! 🎉`)
	} catch (err) {
		errorMessage("Error! ❌")
		console.error(err)
}
...
export const addProduct = async (name: string, price: number, category: string) => {
	try {
		...
		successMessage(`${name} producto agregado! 🎉`)
	}	
		 catch (err) {
		errorMessage("Error! ❌")
		console.error(err)
...
export const addSales = async (customerName: string, customerEmail: string, products: Items[], totalAmount: number, setAddNew: any) => {
	try {
		...
		successMessage("Venta grabada! 🎉")
		setAddNew(false)

	} catch (err) {
		console.error(err)
		errorMessage("Error! reintente nuevamente ❌")
	}
}
