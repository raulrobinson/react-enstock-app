            ...
            <div className='md:w-[85%] w-full py-4 px-6 min-h-[100vh] bg-[#f4f4f6]'>
                ...
                <div className='w-full min-h-[30vh]'>
                    <div className='flex items-center justify-between mb-6'>
                        ...
                        <button className='px-4 py-2 bg-blue-500 text-white rounded' onClick={openModal} >Agregar Nueva</button>
                    </div>
                    <div>
                        {sales.map((sale: Sales) => (
                            <div className='w-full bg-white p-3 flex items-center justify-between rounded my-3' key={sale.id}>
                                <p className='md:text-md text-sm'>Orden{" "}
                                    <span className='text-blue-300'>
                                        #{sale.id}
                                    </span>
                                </p>
                            <Link href="/sales" className='px-4 py-2 bg-[#D64979] text-white text-sm rounded'>Detalles</Link>
                        </div>
                        ))}
                    </div>
                </div>
